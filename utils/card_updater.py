"""
Модуль для обновления карточек игроков.
Вынесен отдельно, чтобы избежать циклических импортов.
"""

from vkbottle import VKAPIError, API
from vkbottle.bot import Message
from datetime import datetime
import os

# Токен администратора для редактирования фото
ADMIN_USER_TOKEN = os.getenv("ADMIN_USER_TOKEN", "")
admin_api = None

if ADMIN_USER_TOKEN:
    admin_api = API(ADMIN_USER_TOKEN)


async def auto_update_card(api, user_db, debug_message: Message = None):
    """
    Обновляет описание фотографии игрока в альбоме группы.
    
    Args:
        api: VK API клиент (не используется, оставлен для совместимости)
        user_db: Объект пользователя из базы данных
        debug_message: Опциональное сообщение для отправки отладочной информации
    
    Требует:
        ADMIN_USER_TOKEN в .env файле - токен администратора группы
    """
    if not user_db.card_photo_id:
        if debug_message:
            await debug_message.answer(
                "╔═════════════════════╗\n"
                "║  ❌ НЕТ ФОТО В БД!   ║\n"
                "╚═════════════════════╝\n\n"
                "В базе данных нет\n"
                "ID фото игрока!\n\n"
                "Привяжи карточку:\n"
                "Связать photo-123_456 @user"
            )
        return

    if not admin_api:
        error_msg = (
            "╔═════════════════════╗\n"
            "║  ⚠️ ОШИБКА ТОКЕНА!   ║\n"
            "╚═════════════════════╝\n\n"
            "Не указан ADMIN_USER_TOKEN!\n\n"
            "Для редактирования описания\n"
            "фото нужен токен админа.\n\n"
            "Добавь в .env файл:\n"
            "ADMIN_USER_TOKEN=ваш_токен"
        )
        print(error_msg, flush=True)
        if debug_message:
            await debug_message.answer(error_msg)
        return

    # Генерируем красивое досье
    karma_status = ""
    if user_db.karma > 0:
        karma_status = f"✨ Уважаемый ({user_db.karma:+d})"
    elif user_db.karma < 0:
        karma_status = f"💩 Гниль ({user_db.karma:+d})"
    else:
        karma_status = "😐 Нейтрал (0)"
    
    dossier_text = (
        f"╔═════════════════════╗\n"
        f"║   🎮 ДОСЬЕ ИГРОКА   ║\n"
        f"╚═════════════════════╝\n\n"
        f"┌─ ЛИЧНЫЕ ДАННЫЕ\n"
        f"│\n"
        f"├─ 👤 {user_db.first_name}\n"
        f"├─ 🎭 {user_db.get_rank()}\n"
        f"│\n"
        f"└─ 💰 {user_db.balance:,} чилликов\n\n"
        f"┌─ РЕПУТАЦИЯ\n"
        f"│\n"
        f"└─ {karma_status}\n\n"
        f"┌─ ЗАРПЛАТА\n"
        f"│\n"
        f"└─ 💳 {user_db.rp_pending_balance:,} чилликов\n\n"
        f"{'═' * 25}\n"
        f"🕒 Обновлено:\n"
        f"   {datetime.now().strftime('%d.%m.%Y %H:%M')}\n"
        f"{'═' * 25}\n\n"
        f"ChillLay RP"
    )

    try:
        # Парсим ID: "-123_456" -> owner_id=-123, photo_id=456
        owner_id, photo_id = map(int, user_db.card_photo_id.split('_'))

        # Используем API администратора для редактирования
        await admin_api.photos.edit(
            owner_id=owner_id,
            photo_id=photo_id,
            caption=dossier_text
        )

        print(f"✅ Описание фото {photo_id} обновлено для {user_db.first_name}.", flush=True)
        if debug_message:
            await debug_message.answer(
                f"╔═════════════════════╗\n"
                f"║  ✅ КАРТОЧКА ОБНОВЛЕНА! ║\n"
                f"╚═════════════════════╝\n\n"
                f"👤 Игрок: {user_db.first_name}\n"
                f"🖼 Фото ID: {photo_id}\n\n"
                f"Досье успешно обновлено! ✨"
            )

    except VKAPIError as e:
        err_msg = getattr(e, "error_msg", str(e))
        err_text = (
            f"╔═════════════════════╗\n"
            f"║  🔥 ОШИБКА VK API!   ║\n"
            f"╚═════════════════════╝\n\n"
            f"Код ошибки: {e.code}\n"
            f"Описание: {err_msg}\n\n"
        )
        
        if e.code == 15:
            err_text += (
                "⚠️ Нет прав доступа!\n\n"
                "Проверь права токена:\n"
                "• Токен должен быть\n"
                "  от администратора\n"
                "• Нужны права на\n"
                "  редактирование фото"
            )
        elif e.code == 100:
            err_text += (
                "⚠️ Неверный ID фото!\n\n"
                "Проверь правильность\n"
                "привязки карточки."
            )
        
        print(err_text, flush=True)
        
        if debug_message:
            await debug_message.answer(err_text)
            
    except Exception as e:
        err_text = (
            f"╔═════════════════════╗\n"
            f"║  🔥 СИСТЕМНАЯ ОШИБКА! ║\n"
            f"╚═════════════════════╝\n\n"
            f"Что-то пошло не так:\n"
            f"{e}\n\n"
            f"Обратись к разработчику!"
        )
        print(err_text, flush=True)
        if debug_message:
            await debug_message.answer(err_text)

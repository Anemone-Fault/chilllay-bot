...
async def transfer(message: Message, match):
    ...
    if target_id == user_db.vk_id: return await message.answer("🤡 Шизофрения? Себе переводить нельзя.", keyboard=kb)
    ...
    await message.answer(f"✅ Перевод выполнен\n💸 -{amount} чилликов\n👤 Получатель: {recipient.first_name}", keyboard=kb)
...
async def create_cheque(message: Message, match):
    ...
    await message.answer(f"╔═══════════════╗\n  🤑 ЧЕК\n╚═══════════════╝\n\n💰 Сумма: {amount}\n👥 Мест: {activations}", keyboard=inline_kb)
...
async def claim_cheque(message: Message):
    ...
    async with in_transaction():
        ...
        if not cheque or cheque.activations_current >= cheque.activations_limit: return await message.answer("❌ Чек пуст.", ephemeral=True)

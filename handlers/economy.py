...
async def bonus_handler(message: Message):
    ...
    text = (
        ...
        f"║   🎁 ПОДАЧКА ВЫДАНА  ║\n"
        f"╚═════════════════════╝\n\n"
        f"💰 Базовая подачка: +{base_amount} чилликов\n"
    )
    
    if karma_bonus > 0:
        text += f"✨ Бонус за карму: +{karma_bonus} чилликов\n"
...
async def casino(message: Message, match):
    ...
    if user_db.balance < bet:
        return await message.answer(
            ...
            f"║   💸 НИЩЕБРОД!       ║\n"
            f"╚═════════════════════╝\n\n"
            f"У тебя всего: {user_db.balance} чилликов\n"
            f"А ты хочешь поставить: {bet} чилликов\n\n"
            f"Математику учил? 🤡",
            keyboard=kb
        )
